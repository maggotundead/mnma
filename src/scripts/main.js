...
fileItemsEditBtns.forEach((btn, index) => {
    btn.addEventListener('click', () => {
        fileItemsEditBtns.forEach((otherBtn, otherIndex) => {
            otherIndex !== index && otherBtn.closest('.file-item').classList.remove('controls-active');
        });
...
document.addEventListener('click', event => {
    ...
    // else if ( !event.target.closest('.sidebar-col') || !event.target.classList.contains('.sidebar-col')) {
    else if ( !event.target.closest('.sidebar-col') ) {
        document.querySelectorAll('.sidebar-col').forEach( sidebar => {
            sidebar.classList.contains('mobile-active') && sidebar.classList.remove('mobile-active');
            // sidebar.classList.contains('mobile-active') && console.log('click outside sidebar');
        });
    }
});
...
const toggleMobileSidebarBtns = document.querySelectorAll('.sidebar-toggler-mobile');
const mobileSidebars = document.querySelectorAll('.sidebar-col');
// console.log(toggleMobileSidebarBtns);

toggleMobileSidebarBtns.forEach( (btn, index) => {
    btn.addEventListener('click', () => {
        mobileSidebars[ index === 0 ? 1 : 0 ].classList.remove('mobile-active');
        mobileSidebars[index].classList.toggle('mobile-active');
    });
});
